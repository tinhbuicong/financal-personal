<script lang="ts">
  ...
  function updateCalendar() {
    ...
    for (let i = 0; i < 42; i++) {
      const iterDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate() + i);
      const isCurrentMonth = iterDate.getMonth() === month;
      
      const dayTransactions = allTransactions.filter(t => {
        const tDate = new Date(t.date);
        return tDate.getFullYear() === iterDate.getFullYear() &&
...
<main class="container">
  ...
  <div class="calendar-header-controls">
    ...
    <div class="month-title-group">
      ...
      <button class="projection-trigger-btn" onclick={() => showProjectionModal = true}>
        📈 Dự tính
      </button>
    </div>
  ...
  <section class="summary-cards">
    ...
    <div class="card">
      ...
      <div class="amount balance">{formatVND(projectionConfig.expected_daily_revenue)}</div>
    </div>
  </section>
  ...
  <div class="main-content">
    <section class="calendar-section">
      ...
      <div class="calendar-grid">
        ...
        {#each calendarDays as day}
          ...
          <div class="calendar-day {day.isCurrentMonth ? 'current-month' : 'other-month'} {day.isToday ? 'is-today' : ''} clickable" onclick={() => openModal(day.date)}>
            <div class="day-number">{day.date.getDate()}</div>
            <div class="day-events">
              {#each day.transactions as t}
                <div class="event-item">
                  <div class="event-desc" title="{t.note ? t.category + ': ' + t.note : t.category}">
                    {t.category}
                  </div>
                  <div class="event-amount {t.type === 'income' ? 'income-text' : 'outcome-text'}">
                    {t.type === 'income' ? '+' : '-'}{(t.amount * 1000).toLocaleString('vi-VN')}đ
                  </div>
                </div>
              {/each}
            </div>
            {#if day.transactions.length > 0}
              <div class="day-total {day.daySum >= 0 ? 'income-bg' : 'outcome-bg'}">
                {day.daySum >= 0 ? '+' : ''} {formatVND(day.daySum)}
              </div>
            {/if}
          </div>
        {/each}
      ...
      <div class="modal-overlay" onclick={closeModal}>
        <div class="modal-content" onclick={(e) => e.stopPropagation()}>
          <div class="modal-header">
            ...
            <button type="button" class="close-btn" aria-label="Close" onclick={closeModal}>&times;</button>
          </div>
          <div class="card form-card">
            <form onsubmit={submitTransaction}>
              ...
              <div class="form-group">
                ...
                <input type="number" id="amount" step="1" min="0" bind:value={amount} required placeholder="0" />
              </div>

              <div class="form-group">
                <label for="category">Danh mục</label>
                <input type="text" id="category" bind:value={category} required placeholder="VD: Ăn uống, Lương..." />
              </div>
              ...
              <div class="form-group">
                ...
                <textarea id="note" bind:value={note} rows="2" placeholder="Chi tiết thêm..."></textarea>
              </div>
    ...
    {#if showProjectionModal}
      <div class="modal-overlay" onclick={() => showProjectionModal = false}>
        <div class="modal-content" onclick={(e) => e.stopPropagation()}>
          <div class="modal-header">
            <h2 class="section-header">Cấu hình Dự tính</h2>
            <button type="button" class="close-btn" onclick={() => showProjectionModal = false}>&times;</button>
          </div>
          <div class="card form-card">
            <div class="form-group">
              <label for="expected">Doanh thu dự kiến hằng ngày (x000VND)</label>
              <input type="number" id="expected" bind:value={projectionConfig.expected_daily_revenue} placeholder="1000" />
            </div>
            <p class="projection-hint">
              Hệ thống sẽ dùng con số này để hiển thị mục tiêu/dự tính cho các ngày tiếp theo.
            </p>
            <button type="button" class="save-config-btn" onclick={updateProjection}>
              Lưu cấu hình
            </button>
          </div>
        </div>
